...
import { adminRequest } from '../../screens/UserScreens/AxiosInstance';
...
const UserOrderTopNavigator = ({ navigation }) => {
    ...
    const [status, setStatus] = useState([
        ...
        { name: 'Expired', id: 5, color: 'orange', bgColor: 'orange', center: 'center', textColor: 'white' },
        { name: 'All', id: 6, color: 'black', bgColor: '#DDDDDD', textColor: 'black' },
    ]);
    ...
    //     try {
    //         // console.log('GET ORDER APIII', `/api/order/orderListbyuser?limit=3&startDate=${startDatee}&page=1&status=${filterStatus}&time=[${data}]&endDate=${newEndDatee}`);
    ...
    const GetOrders = async () => {
        if (orderDetail) {
            let body = {
                ...
                timeArr: null  // 24 hour format 12am to 1am will be 0 and it will end on 23. i.e. 11pm to 12am
            }
            console.log('RESPONSE FROM ORDER BODY', body);
            try {
                const resp = await adminRequest.post('/api/order/orderListbyuserId', body)
                // console.log('RESPONSE FROM ORDER API', resp.data.msg);
                ...
                console.log('ERROR FROM GET ORDER API', error);
    ...
    }, [startDatee, newEndDatee, data, filterStatus, userDetail?._id])
        ...
        return (
            <View style={styles.containerBox}>
                ...
                <View style={{ flexDirection: 'row-reverse', bottom: '8%', right: '5%' }}>
                    <TouchableOpacity onPress={() => navigation.navigate('UserDetailScreen', { OrderDetail: item?._id })}>
                ...
                <View style={{ bottom: '10%' }}>
                    <View style={{ flexDirection: 'row' }}>
                        ...
                        {item?.totalViews > 0 ?
                            <View style={{ borderWidth: 1, width: 80, height: 26, borderRadius: 15, borderColor: 'rgba(183,54,248,255)', left: 60, bottom: 15 }}>
                                <View style={{ flexDirection: 'row' }}>
                                    <Image source={images.eye} style={{ tintColor: 'rgba(183,54,248,255)', left: 5 }} />
                                    <Text style={{ color: 'rgba(183,54,248,255)', fontFamily: 'Oswald-Bold', left: 20 }}>{item.totalViews}</Text>
                                </View>
                            </View>
                            :
                            null
                    ...
                    <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginLeft: 12, marginRight: 12 }}>
                        ...
                        <Text style={styles.duration}>{item.videoDuration / 30} times</Text>
                    </View>
        ...
        <View style={styles.main}>
            ...
            <View>
                <View style={{ height: 78, backgroundColor: 'rgba(183,54,248,255)' }}>
                    ...
                    <View>
                        <Text style={{ alignSelf: 'center', bottom: 19, fontFamily: 'Oswald-Bold', color: 'white', fontSize: 18 }}>Orders</Text>
                    </View>
                </View>
            </View>
...
export default UserOrderTopNavigator;
