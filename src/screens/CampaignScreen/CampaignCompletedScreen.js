...
import { StatusBar, Text, View, FlatList, Image, StyleSheet, TextInput, TouchableOpacity, ScrollView, RefreshControl } from 'react-native'
...
const CampaignCompleted = ({ navigation }) => {
    ...
    const getCompletedCampaignData = async () => {
        try {
            let res = await axios.post(`api/campaign/getcampaignListbyUserId?limit=100&date=2022-08-30&page=1&status=completed`)
            console.log("GET COMPLETED CAMPAIGN RESPONSE ==> ", res.data);
            setCompleted(res.data.msg)
        } catch (error) {
            console.log('GET COMPETED CAMPAIGN ERROR', error);

        }
    }
    useEffect(() => {
        getCompletedCampaignData()
