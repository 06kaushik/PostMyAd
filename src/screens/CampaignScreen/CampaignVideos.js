...
const CampaignVideo = ({ navigation, route }) => {
    ...
    const phoneVideo = ({ item }) => {
        return (
            <View style={{
                ...
                <TouchableOpacity onPress={() => navigation.navigate('CampaignSummary',{videoLink: item?.compressedlink,date: GalleryVideo?.date, time: GalleryVideo?.time, endtime: GalleryVideo?.endTime, MacId: GalleryVideo?.MacId, deviceId: GalleryVideo?.deviceId, adTitle: GalleryVideo?.adTitle, aboutAd: GalleryVideo?.aboutAd, billBoardId: GalleryVideo?.billBoardId,videoname:item?.videoname,videoFileType:'video/mp4', videoDate: item?.date,billboardname: GalleryVideo?.billboardname, billboardaddress:GalleryVideo?.billboardaddress,uniquenamevideoname: item.uniquename})} >
