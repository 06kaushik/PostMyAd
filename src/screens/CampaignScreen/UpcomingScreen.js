...
const UpcomingScreen = ({ navigation }) => {
    ...
    const getUpcommingCampaignData = async () => {
        try {
            let res = await axios.post(`api/campaign/getcampaignListbyUserId?limit=100&date=${moment().format("YYYY-MM-DD")}&page=1&status=upcoming`)
            setCampaignData(res.data.msg)
        } catch (error) {
            console.log('GET UPCOMMING CAMPAIGN ERROR', error);

        }
    }
    useEffect(() => {
        getUpcommingCampaignData()
