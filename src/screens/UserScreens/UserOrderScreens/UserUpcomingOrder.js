...
const UserUpcomingOrder = () => {

    const [approved, setApproved] = useState([])
    console.log('PENDING DATA', approved);
    const isFocused = useIsFocused();

    const GetOrders = async () => {
        try {
            let res = await axios.get('/api/order/orderListforBusinessOwner?&status=approved')
            console.log('API DATA APPROVED', res.data.msg);
            setApproved(res.data.msg)
        } catch (error) {
            console.log('GET APPROVED ORDER ERROR', error);
        }
    }
