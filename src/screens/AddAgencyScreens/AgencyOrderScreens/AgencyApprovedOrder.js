...
const AgencyApprovedOrder = ({navigation}) => {
    const [upcomingOrder, setUpcomingOrder] = useState([])
    console.log('PENDING DATAAA', upcomingOrder);
    ...
    const getUpcommingOrderData = async () => {
        try {
            let res = await axios.get('/api/order/orderListforBusinessOwner?&status=Pending')
            console.log("GET UPCOMMING ORDER RESPONSE IN OVERVIEW ==> ", res.data.msg);
...
export default AgencyApprovedOrder;
